-- Phase B v2: valid hex UUIDs throughout
...
-- ============================================================
INSERT INTO marketplace_connections (id, tenant_id, marketplace, account_name, credentials, status, last_sync_at, sync_config)
VALUES
...
-- ============================================================
-- WORKFLOWS (use valid hex: 0f,1f,2f,3f,4f prefix)
-- ============================================================
INSERT INTO workflows (id, name, description, trigger_type, trigger_config, steps, is_active, last_run_at, run_count)
VALUES
  ('0f000001-0000-0000-0000-000000000001'::UUID,
   'Low Stock Alert & Reorder',
   'Auto-detect products below reorder point and create PO draft',
   'schedule',
   '{"cron": "0 9 * * *", "timezone": "America/New_York"}'::jsonb,
   '[{"step": 1, "action": "check_inventory", "config": {"threshold": "reorder_point"}}, {"step": 2, "action": "create_po_draft", "config": {"auto_submit": false}}, {"step": 3, "action": "notify", "config": {"channels": ["email", "in_app"]}}]'::jsonb,
   true, NOW() - INTERVAL '6 hours', 47),

  ('0f000002-0000-0000-0000-000000000002'::UUID,
   'Daily PPC Budget Optimizer',
   'Adjust campaign budgets based on ROAS performance each morning',
   'schedule',
   '{"cron": "0 7 * * 1-5", "timezone": "America/New_York"}'::jsonb,
   '[{"step": 1, "action": "fetch_ppc_metrics", "config": {"lookback_days": 7}}, {"step": 2, "action": "compute_optimal_bids", "config": {"target_acos": 20}}, {"step": 3, "action": "apply_bid_changes", "config": {"dry_run": false}}, {"step": 4, "action": "notify", "config": {"channels": ["in_app"]}}]'::jsonb,
   true, NOW() - INTERVAL '1 hour', 124),

  ('0f000003-0000-0000-0000-000000000003'::UUID,
   'Competitor Price Watch',
   'Monitor competitor prices and alert when undercutting by >10%',
   'event',
   '{"event": "competitor_price_change", "threshold_pct": 10}'::jsonb,
   '[{"step": 1, "action": "fetch_competitor_prices", "config": {"marketplaces": ["amazon"]}}, {"step": 2, "action": "compare_prices", "config": {"alert_threshold_pct": 10}}, {"step": 3, "action": "notify", "config": {"channels": ["email", "in_app"], "priority": "high"}}]'::jsonb,
   true, NOW() - INTERVAL '3 hours', 89),

  ('0f000004-0000-0000-0000-000000000004'::UUID,
   'Weekly AI Listing Audit',
   'Run SEO audit on all listings every Sunday and queue optimizations',
   'schedule',
   '{"cron": "0 10 * * 0", "timezone": "America/New_York"}'::jsonb,
   '[{"step": 1, "action": "list_active_listings", "config": {}}, {"step": 2, "action": "run_seo_audit", "config": {"min_score_threshold": 80}}, {"step": 3, "action": "queue_optimizations", "config": {"auto_apply": false}}, {"step": 4, "action": "notify", "config": {"channels": ["email"], "include_report": true}}]'::jsonb,
   true, NOW() - INTERVAL '6 days', 12),

  ('0f000005-0000-0000-0000-000000000005'::UUID,
   'Trend-to-Product Pipeline',
   'When a viral trend detected (score >85), auto-research sourcing options',
   'event',
   '{"event": "trend_detected", "min_viral_score": 85}'::jsonb,
   '[{"step": 1, "action": "analyze_trend", "config": {"depth": "full"}}, {"step": 2, "action": "search_suppliers", "config": {"countries": ["CN", "VN"], "max_results": 5}}, {"step": 3, "action": "estimate_margins", "config": {"target_roi": 40}}, {"step": 4, "action": "create_opportunity_report", "config": {}}, {"step": 5, "action": "notify", "config": {"channels": ["email", "in_app"], "priority": "high"}}]'::jsonb,
   false, NOW() - INTERVAL '2 days', 3)
ON CONFLICT (id) DO NOTHING;

-- ============================================================
-- AGENT_RUNS (use valid hex: ab prefix = valid)
-- ============================================================
INSERT INTO agent_runs (id, agent_type, status, input, output, steps, tokens_used, cost_usd, started_at, completed_at)
VALUES
  ('ab000001-0000-0000-0000-000000000001'::UUID,
   ...
   '[{"step": "scrape_categories", "duration_ms": 3200}, {"step": "score_products", "duration_ms": 1800}]'::jsonb,
  ...
  ('ab000002-0000-0000-0000-000000000002'::UUID,
   ...
   '[{"step": "scrape_categories", "duration_ms": 2900}, {"step": "score_products", "duration_ms": 1600}]'::jsonb,
   3940, 0.0118, NOW() - INTERVAL '4 hours', NOW() - INTERVAL '4 hours' + INTERVAL '49 seconds'),

  ('ab000003-0000-0000-0000-000000000003'::UUID,
   'trend', 'completed',
   ...
   6210, 0.0186, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '1 hour' + INTERVAL '72 seconds'),

  ('ab000004-0000-0000-0000-000000000004'::UUID,
   'trend', 'completed',
   ...
   '[{"step": "scan_reddit", "duration_ms": 3600}, {"step": "scan_pinterest", "duration_ms": 3100}]'::jsonb,
   5480, 0.0164, NOW() - INTERVAL '5 hours', NOW() - INTERVAL '5 hours' + INTERVAL '67 seconds'),

  ('ab000005-0000-0000-0000-000000000005'::UUID,
   ...
   9830, 0.0295, NOW() - INTERVAL '3 hours', NOW() - INTERVAL '3 hours' + INTERVAL '118 seconds'),

  ('ab000006-0000-0000-0000-000000000006'::UUID,
   'competitor', 'completed',
   ...
   '[{"step": "fetch_asins", "duration_ms": 1100}, {"step": "mine_reviews", "duration_ms": 7900}]'::jsonb,
   8640, 0.0259, NOW() - INTERVAL '8 hours', NOW() - INTERVAL '8 hours' + INTERVAL '104 seconds'),

  ('ab000007-0000-0000-0000-000000000007'::UUID,
   'inventory', 'completed',
  ...
  ('ab000008-0000-0000-0000-000000000008'::UUID,
   'inventory', 'completed',
   '{"check_stockout": true, "check_overstock": true}'::jsonb,
   '{"stockouts_predicted": 2, "overstock_items": 0, "reorder_value": 8100}'::jsonb,
   '[{"step": "fetch_inventory", "duration_ms": 750}, {"step": "calc_velocity", "duration_ms": 1100}]'::jsonb,
   2050, 0.0062, NOW() - INTERVAL '6 hours', NOW() - INTERVAL '6 hours' + INTERVAL '25 seconds'),

  ('ab000009-0000-0000-0000-000000000009'::UUID,
   'ppc', 'completed',
   ...
   '[{"step": "fetch_campaigns", "duration_ms": 1400}, {"step": "analyze_performance", "duration_ms": 2200}, {"step": "compute_bids", "duration_ms": 1800}]'::jsonb,
   7340, 0.0220, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '1 hour' + INTERVAL '125 seconds'),

  ('ab00000a-0000-0000-0000-000000000010'::UUID,
   'ppc', 'completed',
   '{"optimization_goal": "roas", "target_roas": 5, "campaigns": "all"}'::jsonb,
   '{"campaigns_analyzed": 6, "bids_adjusted": 3, "projected_savings": 98, "projected_revenue_lift": 620}'::jsonb,
   '[{"step": "fetch_campaigns", "duration_ms": 1350}, {"step": "analyze_performance", "duration_ms": 2100}]'::jsonb,
   6980, 0.0209, NOW() - INTERVAL '7 hours', NOW() - INTERVAL '7 hours' + INTERVAL '118 seconds'),

  ('ab00000b-0000-0000-0000-000000000011'::UUID,
   'supplier', 'completed',
   '{"action": "risk_assessment", "supplier_ids": ["bbbb0001-0000-0000-0000-000000000001"]}'::jsonb,
   '{"suppliers_analyzed": 2, "risk_flags": 1, "flag": "FitGear MOQ too high for Q4 demand spike", "estimated_cost_savings": 1840}'::jsonb,
   '[{"step": "fetch_suppliers", "duration_ms": 900}, {"step": "assess_risks", "duration_ms": 3200}]'::jsonb,
   8120, 0.0244, NOW() - INTERVAL '12 hours', NOW() - INTERVAL '12 hours' + INTERVAL '98 seconds'),

  ('ab00000c-0000-0000-0000-000000000012'::UUID,
   'listing', 'completed',
   ...
   '[{"step": "fetch_listings", "duration_ms": 600}, {"step": "audit_seo", "duration_ms": 4800}]'::jsonb,
   11240, 0.0337, NOW() - INTERVAL '4 hours', NOW() - INTERVAL '4 hours' + INTERVAL '94 seconds'),

  ('ab00000d-0000-0000-0000-000000000013'::UUID,
   'listing', 'completed',
   '{"action": "keyword_refresh", "marketplace": "amazon"}'::jsonb,
   '{"keywords_refreshed": 5, "new_keywords_discovered": 8, "top_opportunity": "bamboo kitchen organizer set"}'::jsonb,
   '[{"step": "fetch_keywords", "duration_ms": 700}, {"step": "research_new", "duration_ms": 5200}]'::jsonb,
   9870, 0.0296, NOW() - INTERVAL '9 hours', NOW() - INTERVAL '9 hours' + INTERVAL '82 seconds'),

  ('ab00000e-0000-0000-0000-000000000014'::UUID,
...
-- ============================================================
-- NOTIFICATIONS (use valid hex: ac prefix)
-- ============================================================
INSERT INTO notifications (id, user_id, type, title, body, action_url, metadata, is_read, created_at)
VALUES
  ('ac000001-0000-0000-0000-000000000001'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
   'stockout_alert', 'Stockout Alert: Water Bottle 32oz',
  ...
  ('ac000002-0000-0000-0000-000000000002'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
   'stockout_alert', 'Stockout Alert: LED Desk Lamp',
  ...
  ('ac000003-0000-0000-0000-000000000003'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
  ...
  ('ac000004-0000-0000-0000-000000000004'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
   'ppc_alert', 'PPC: ACoS Spike on Silicone Utensil Auto',
   'Silicone Cooking Utensil Auto campaign ACoS hit 24.6% (target: 20%). Budget exhausted by 2pm daily.',
   '/ppc', '{"campaign_id": "AMZ-CAMP-1003", "acos": 24.6, "target_acos": 20, "action": "increase_budget"}'::jsonb,
   false, NOW() - INTERVAL '2 hours'),

  ('ac000005-0000-0000-0000-000000000005'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
   'competitor_alert', 'Competitor Price Drop: BambooKing to $28.99',
  ...
  ('ac000006-0000-0000-0000-000000000006'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
  ...
  ('ac000007-0000-0000-0000-000000000007'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
  ...
  ('ac000008-0000-0000-0000-000000000008'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
   'agent_complete', 'Agent Complete: Competitor Analysis',
   'Competitor analysis for Bamboo Cutting Board complete. Found 6 weaknesses in top competitors.',
   '/competitors', '{"agent": "competitor", "product": "Bamboo Cutting Board", "weaknesses_found": 6}'::jsonb,
   true, NOW() - INTERVAL '8 hours'),

  ('ac000009-0000-0000-0000-000000000009'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
   'ppc_alert', 'PPC Optimized: Campaigns Updated',
   'AI adjusted bids on 4 campaigns. Projected ACoS improvement: 24.6% to 19.8%. Estimated +$890/mo revenue.',
   '/ppc', '{"campaigns_updated": 4, "projected_acos": 19.8, "revenue_lift": 890}'::jsonb,
   true, NOW() - INTERVAL '1 hour'),

  ('ac00000a-0000-0000-0000-000000000010'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
  ...
  ('ac00000b-0000-0000-0000-000000000011'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
  ...
  ('ac00000c-0000-0000-0000-000000000012'::UUID,
   '8e63fa3b-4d49-4943-aeb7-ebf1b41e4825'::UUID,
